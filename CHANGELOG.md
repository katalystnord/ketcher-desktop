...
### Included from 3.17.2

- The build-pipeline work done for the 3.17.2 stable release ships here too: `npm ci` instead of `npm install`, a TypeScript-version-aware `moduleResolution`, and the `DISABLE_ESLINT_PLUGIN` example build. The two Ketcher 3.17.x-specific patches (`fix-ketcher-jsx-namespace.cjs`, `fix-ketcher-rpt2-check.cjs`) detect that they are not needed on 3.19 and skip themselves. See the 3.17.2 entry below for why each exists.
